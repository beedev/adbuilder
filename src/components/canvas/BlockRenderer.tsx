...
export function BlockRenderer({ placedBlock, scale, isSelected, onSelect, mode = 'edit' }: Props) {
  ...
  // ── Sale Band mode ──────────────────────────────────────────────────
  // Standalone promotional block (image 11): red band + price circle + ad copy.
  // Also works for product blocks with sale_band display mode.
  if (displayMode === 'sale_band') {
    const bandBg = bgColor === '#FFFFFF' ? '#C8102E' : bgColor
    ...
    return (
      ...
      >
        {/* Left: price circle (product price OR free-form promo price) */}
        {promoPrice ? (
          <div
            style={{
        ...
        <div style={{ flex: 1, minWidth: 0, overflow: 'hidden' }}>
          {headline && (
            <div style={{ fontSize: Math.max(9, 14 * scale), fontWeight: 900, color: '#fff', lineHeight: 1.15, marginBottom: 2, overflow: 'hidden', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' }}>
              {headline}
            </div>
          )}
          {description && (
            <div style={{ fontSize: Math.max(7, 10 * scale), color: 'rgba(255,255,255,0.88)', lineHeight: 1.3, overflow: 'hidden', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' }}>
              {description}
            </div>
          )}
          {disclaimer && (
            <div style={{ fontSize: Math.max(6, 7 * scale), color: 'rgba(255,255,255,0.65)', lineHeight: 1.2, marginTop: 2, fontStyle: 'italic' }}>
              {disclaimer}
            </div>
          )}
        </div>
